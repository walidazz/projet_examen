...
<li class="active">Les articles les plus appréciés par la communauté</li>
</ol>
            ...
            <li class=" bg-article list-group-item  mb-2 bg-article text-dark view zoom " style="color:white;" >
                <div class="row ">
                    ...
                    <div class="col-xs-2 col-md-2 p-1  ">
                        <img style="height:150px;width:150px;" src="<?= WEBROOT. $article->getImg() ?>" class=" img-circle border-white img-responsive imgArticle" alt="image article" /></div>
...
<nav aria-label="Page">
  ...
  <?php 	for($i=1; $i<=$nbPage; $i++){
	  if($i == $current){ ?>
	  <li class="page-item"><a class="page-link active" href="<?= WEBROOT?>Article/bestseller/<?=$i?>"> <?=$i?>   </a></li> <?php }  else {?>

<li class="page-item"><a class="page-link" href="<?= WEBROOT?>Article/bestseller/<?=$i?>"> <u> <?=$i?></u> </a></li> <?php } } ?>
